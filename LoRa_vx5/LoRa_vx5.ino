...
void setup() {
  ...
  if (configured == true) {
    ...
    do { 
      ...
      while (standby == true & (millis() - standByTimer) < (relayModeTime - delay1)) {
        receivePacket();
      }
      if (packet.command == 0) {
//        if(sleepMinute != packet.sleepTime){
...
if (configured == false) {
  ...
  if (packet.command == 0) { //set level and sleep time
    standby = true;
    stringPacket("Received level and sleep time");
    sleepMinute = packet.sleepTime;
    if (packet.level < level) {
      level = packet.level+1;
    }

    stringPacket("level: " + String(level) + "..." + "sleep time: " + String(sleepMinute));
    levelSet = true;
    sleepTimeSet = true;
    standby = true;
    //sendPacket(packet);
  ...
  if (packet.command == 1) { //go to sleep command  
    if (sleepTimeSet == true & levelSet == true) {
      stringPacket("Entering sleep mode...");
      configured = true;
      alarmMode = DS3231_A1_Minute;
